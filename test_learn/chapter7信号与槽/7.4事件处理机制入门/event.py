# -*- coding:UTF-8 -*-
'''
事件处理的经典案例，对于事件处理常用的两种方法都有涉及
内容丰富，基本包含了我们对事件处理的绝大部分需求
１）重新实现事件函数，如mousePressEvent(),keyPressEvent(),paintEvent()
２）重新实现QObject.event()
...
class Widget(QWidget):
    def __init__(self,parent=None):
        ...
        #避免受窗口大小重绘事件的影响，可以把参数０改成３０００（３秒），然后再运行，就可以明白这行代码的意思
        QTimer.singleShot(3000,self.giveHelp)

    def giveHelp(self):
        self.text="请点击这里触发追踪鼠标功能"
        #重绘时间，也就是触发paintEvent事件
        self.update()
    ...
    '''重新实现上下文菜单事件'''
    def contectMenuEvent(self,event):
        menu=QMenu(self)
    ...
    def paintEvent(self,event):
        ...
        #若信息文本存在，则在底部居中绘制信息，５秒后清空信息文本并重绘
        if self.message:
    ...
    '''
    第二种事件处理方法：重载event函数
    重新实现其他事件，适用于PyQt没有提供该事件的处理函数的情况，Tab键由于涉及焦点切换
    不会传递给keyPressEvent，因此需要在这里重新定义
    '''
